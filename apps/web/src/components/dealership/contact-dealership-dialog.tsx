...
interface ContactDealershipDialogProps {
	dealershipId: string;
...
export function ContactDealershipDialog({ dealershipId, dealershipName, triggerButton }: ContactDealershipDialogProps) {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		setIsLoading(true);
		try {
			const response = await apiFetch(`/dealerships/${dealershipId}/contact`, {
				method: "POST",
				...
				{isSuccess ? (
					<div className="py-8 flex flex-col items-center justify-center text-center gap-4">
						...
						<div>
							...
							<p className="text-muted-foreground mt-2">
								{t("contact.successDesc", { title: dealershipName })}
							</p>
						</div>
					...
					<form onSubmit={handleSubmit}>
						<DialogHeader>
							...
							<DialogDescription>
								{t("contact.dialogDescription", { title: dealershipName })}
							</DialogDescription>
						</DialogHeader>
